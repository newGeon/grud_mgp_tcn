...
ex = Experiment('V2_MGP-TCN-0-BASIC')
...
def get_GP_samples(Y, T, X, ind_kf, ind_kt, num_obs_times, num_obs_values,
    ...
    def body(i, Z):
        Yi = tf.reshape(tf.slice(Y, [i, 0], [1, num_obs_values[i]]), [
            -1])  # MM: tf.reshape(x, [-1]) flattens tensor x (e.g. [2,3,1] to [6]), slice cuts out all Y data of one patient
...
def mpg_tcn_config():
    dataset = {
        ...
        'datapath': 'output_imputation/output_0/',
        ...
        'LOG_FILE': 'v2_0_basic_0_55h_ex1c.txt',
        'hour': '55h',
        'kind': 'ex1c'
    }
    # Padding method:
...
def get_dataset(na_thres, overwrite, horizon, data_sources, hour, kind, min_length, max_length, split, num_obs_thres,
    ...
    datapath += 'mgp-tcn-datadump_{}_{}_na_thres_{}_min_length_{}_max_length_{}_horizon_{}_split_{}.pkl'.format(hour, kind, na_thres, min_length, max_length, horizon, 0)
...
def fit_mgp_tcn(decomposition_method, add_diag, losstype, n_hidden, levels, kernel_size, n_mc_smps, dropout,
    ...
    checkpoint_path = dataset['root_path'] + 'model_v2/checkpoints_0_basic'
    if not os.path.isdir(checkpoint_path):
        os.mkdir(checkpoint_path)

    checkpoint_path = checkpoint_path + '/' + checkpoint_folder

    result_image_path = dataset['root_path'] + 'model_v2/result_image_0_basic'
    print("---------------------------------------------------------------------------------------------------")
    ...
    for horizon in range(0, 8):
        # Load dataset
        # 이미 full_dataset 이전에 데이터 변경되어서 들어옴 (GP 실행 전 데이터, 단순하게 표준화 등 변경된 데이터)
        full_dataset = get_dataset(dataset['na_thres'], dataset['overwrite'], horizon, dataset['data_sources'],
        ...
        if 'labs' and 'vitals' in data_sources:
            ...
            raise ValueError(
                'Labs or Vitals not selected in config, yet they are required')  # TODO: make this case possible
        ...
        ind_times_tr = train_data[3];labels_tr
        ...
        for i in range(training_iters):
            ...
            for s, e in zip(starts, ends):
                ...
                print("Batch " + "{:d}".format(batch) + "/" + "{:d}".format(num_batches) + ", took: " + "{:.3f}".format(
                    time() - batch_start) + ", loss: " + "{:.5f}".format(loss_))

                write_file = open(log_path + log_file, 'a+')
                write_file.write("train_loss,{},{},{},{},{:d},{:.5f}\n".format(dataset['split'], dataset['hour'],
                                                                               dataset['kind'], horizon, i, loss_))
                write_file.close()
                ...
                if total_batches % test_freq == 0:  # Check val set every so often for early stopping
                    ...
                    write_file.write(
                        "val_loss,{},{},{},{},{:d},{:.5f},{:.5f},{:.5f}\n".format(dataset['split'], dataset['hour'],
                                                                                  dataset['kind'], horizon, i,
                                                                                  va_loss, va_prc, va_auc))
                    write_file.close()
                    ...
                    saver.save(sess, checkpoint_path + "/model_{}_{}_split_{}_hor_{}_epoch_{}".format(dataset['hour'], dataset['kind'], dataset['split'], horizon, i), global_step=total_batches)
            ...
            tr_roc_auc_png = tr_result_image_path + "/tr_roc_curve_{}_{}_split_{}_hor_{}_epoch_{}_{}.png".format(
                dataset['hour'], dataset['kind'], dataset['split'], horizon, i, total_batches)
            ...
            write_file.write(
                "final_train_loss,{},{},{},{},{:d},{:.5f},{:.5f},{:.5f}\n".format(dataset['split'], dataset['kind'],
                                                                                  dataset['split'], horizon, i,
                                                                                  loss_, tr_prc, tr_auc))
            write_file.write(
                "best,{},{},{},{},{},{},{:.5f},{:.5f}\n".format(dataset['split'], dataset['kind'], dataset['split'],
                                                                horizon, labels_tr.shape, labels_va.shape,
                                                                va_best_val, va_best_auroc))
            write_file.close()

            print("Finishing epoch " + "{:d}".format(i) + ", took " + "{:.3f}".format(time() - epoch_start) + 'horizon = ' + str(horizon))
            print("best,{},{},{},{},{:.5f},{:.5f}\n".format(dataset['split'], horizon, labels_tr.shape, labels_va.shape, va_best_val, va_best_auroc))
            print("final_train_loss,{},{},{:d},{:.5f},{:.5f},{:.5f}\n".format(dataset['split'], horizon, i, loss_, tr_prc, tr_auc))
