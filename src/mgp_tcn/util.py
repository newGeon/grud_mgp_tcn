...
def mask_large_samples(data, thres, obs_min, static=None):
    ...
    statistics = {'total_remain': total_remain, 'total_removed': total_removed,
                  'removed_to_few_obs': removed_to_few_obs}
...
def select_horizon(data, horizon):
    ...
    results = [np.array(item) for item in
               [values, times, ind_lvs, ind_times, labels, num_rnn_grid_times, rnn_grid_times, num_obs_times,
                num_obs_values, onset_hour]]
    return results


def pad_rawdata_nomed(T, Y, ind_kf, ind_kt, X, covs, num_rnn_grid_times,
                      min_pad_length):  ## with med: meds_on_grid,covs -- now with: num_rnn_grid_times
    """
    ...
    for i in range(N):
        ...
        meds_cov_pad[i, :grid_lens[i], :num_covs] = np.tile(covs[i], (
        grid_lens[i], 1))  ## meds_cov_pad[i,:grid_lens[i],num_meds:]
