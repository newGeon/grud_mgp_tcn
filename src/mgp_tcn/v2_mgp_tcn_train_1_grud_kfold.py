...
# sys.path.append("/home/cdss/mgp-tcn-project/src/mgp_tcn")

from memory_saving_gradients import gradients_memory
from tcn import CausalConv1D, TemporalBlock, TemporalConvNet
from util import select_horizon, pad_rawdata_nomed, SE_kernel, OU_kernel, dot, CG, Lanczos, block_CG, block_Lanczos
...
ex = Experiment('V2_MGP-TCN-1-GRUD-KFOLD')
...
def compute_l1():
    ...
    kernel_abs_sum = tf.reduce_sum(kernel_abs,
                                   axis=0)  # sum over kernel size, such that each entry represents absolute sum of all filter values for one channel and one filter
    bias_abs_sum = tf.reduce_sum(bias_abs)

    l1_per_filter = tf.reduce_sum(kernel_abs_sum,
                                  axis=0)  # compute l1 over all channels per filter, since all elements are positive: sum over each channel
...
def draw_GP(Yi, Ti, Xi, ind_kfi, ind_kti, method, gp_params):
    ...
    if method.methodname == 'chol':
        ...
        Sigma = K_ff - tf.matmul(K_fy, tf.cholesky_solve(Ly, tf.transpose(K_fy))) + method.add_diag * tf.eye(
            tf.shape(K_ff)[0])
...
def get_GP_samples(Y, T, X, ind_kf, ind_kt, num_obs_times, num_obs_values,
                   num_tcn_grid_times, cov_grid, input_dim, method, gp_params, lab_vitals_only,
                   pad_before):  ##,med_cov_grid
    """
...
def get_preds(Y, T, X, ind_kf, ind_kt, num_obs_times, num_obs_values,
              num_tcn_grid_times, cov_grid, input_dim, method, gp_params, tcn, is_training, n_classes, lab_vitals_only,
              pad_before, losstype):  # med_cov_grid
    """
    ...
    Z = get_GP_samples(Y, T, X, ind_kf, ind_kt, num_obs_times, num_obs_values,
                       num_tcn_grid_times, cov_grid, input_dim, method=method, gp_params=gp_params,
                       lab_vitals_only=lab_vitals_only,
                       pad_before=pad_before)  # batchsize*num_MC x batch_maxseqlen x num_inputs    ##,med_cov_grid
...
def get_losses(Y, T, X, ind_kf, ind_kt, num_obs_times, num_obs_values,
    ...
    all_losses = tf.nn.weighted_cross_entropy_with_logits(logits=tcn_logits, targets=tiled_labels,
...
def get_probs_and_accuracy(preds, O, n_mc_smps):
    ...
    """
    all_probs = tf.exp(
        preds[:, 1] - tf.reduce_logsumexp(preds, axis=1))  # normalize; and drop a dim so only prob of positive case
    N = tf.cast(tf.shape(preds)[0] / n_mc_smps,
                tf.int32)  # actual number of observations in preds, collapsing MC samples
...
def mpg_tcn_config():
    dataset = {
        ...
        'datapath': 'output_imputation/output_1_kfold_5/',
        ...
        'split': 0,                 # 0-4
        'root_path': '/home/cdss/grud_mgp_tcn/',
        'LOG_FILE': 'v2_1_grud_1_55h_ex1c.txt',
        'hour': '55h',
        'kind': 'ex1c'
    }
    # Padding method:
    ...
    add_diag = 0.1  # changed from 1e-3 for more stability
    ...
    levels = 9
    kernel_size = 5
    n_hidden = 56  # hidden layer num of features; assumed same
    # n_layers = 1       # 3 number of layers of stacked LSTMs
    n_mc_smps = 10  # 10
    dropout = 0.0  # not keep-dropout, but % of dropping out!
    ...
    L2_penalty = 0.01  # using per-weight norm! hence lambda so large.. multiplied with start per weight-norm value arrives at around 10. train loss around 100-2000
...
def get_dataset(na_thres, overwrite, horizon, kfold, data_sources, hour, kind, min_length, max_length, split, num_obs_thres,
                root_path, output_path):
    print('----- get_dataset ------------------------------------------------------------')
    print('------------------------------------------------------------------------------')
    print('------------------------------------------------------------------------------')
    print('na_thres = ' + str(na_thres))
    ...
    print('num_obs_thres = ' + str(num_obs_thres))
    print('root_path = ' + root_path)
    print('output_pnath = ' + output_path)
    print('------------------------------------------------------------------------------')
    print('------------------------------------------------------------------------------')
    print('------------------------------------------------------------------------------')

    datapath = root_path + output_path

    print('USING SPLIT {}, HORIZON {}, KFOLD {}'.format(split, horizon, kfold))
    # data load 관련으로는 split와 관계 없이 동일한 데이터 적용
    # mgp-tcn-datadump_55h_horizon_0_kfold_0.pkl
    datapath += "mgp-tcn-datadump_{}_horizon_{}_kfold_{}.pkl".format(hour, horizon, kfold)
...
def dataset_statistics(na_thres, datapath, horizon, data_sources, min_length, max_length, split):
    ...
    datapath += 'mgp-tcn-datadump_' + '_'.join(
        [str(el) for el in data_sources]) + '_na_thres_{}_min_length_{}_max_length_{}_horizon_{}_split_{}.pkl'.format(
        na_thres, min_length, max_length, horizon, split)
...
@ex.main
def fit_mgp_tcn(decomposition_method, add_diag, losstype, n_hidden, levels, kernel_size, n_mc_smps, dropout,
                reduction_dim, batch_size, learning_rate, decay_learning_rate,
                training_iters, time_reset, l1_filter_reg, drop_first_res, L1_penalty, L2_penalty, pad_before, _rnd,
                _seed, _run, dataset):
    log_path = dataset['root_path'] + 'log_file_v2/'
    log_file = dataset['LOG_FILE']

    checkpoint_folder = log_file.replace('.txt', '_checkpoints')

    checkpoint_path = dataset['root_path'] + 'model_v2/checkpoints_1_grud_kfold'
    if not os.path.isdir(checkpoint_path):
        os.mkdir(checkpoint_path)

    checkpoint_path = checkpoint_path + '/' + checkpoint_folder

    result_image_path = dataset['root_path'] + 'model_v2/result_image_1_grud_kfold'
    print("---------------------------------------------------------------------------------------------------")
    ...
    for horizon in range(3, 4):
        for kfold in range(3, 5):
            # Load dataset
            # 생성된 full_dataset 이전에 데이터 변경되어서 들어옴 (GP 실행 전 데이터, 단순하게 표준화 등 변경된 데이터)
            full_dataset = get_dataset(dataset['na_thres'], dataset['overwrite'], horizon, kfold, dataset['data_sources'],
                                       dataset['hour'], dataset['kind'], dataset['min_length'], dataset['max_length'],
                                       dataset['split'], dataset['num_obs_thres'], dataset['root_path'], dataset['datapath'])
            ...
            elif 'labs' or 'vitals' in data_sources:
                if num_obs_thres is not None:
            ...
            if time_reset:
                # time_reset = 0 >> false 이기 때문에 실행 안 함
            ...
            # Assign data splits:
            values_tr = train_data[0]
            values_va = validation_data[0]
            times_tr = train_data[1]
            times_va = validation_data[1]
            ind_lvs_tr = train_data[2]
            ind_lvs_va = validation_data[2]
            ind_times_tr = train_data[3]
            ind_times_va = validation_data[3]
            labels_tr = train_data[4]
            labels_va = validation_data[4]
            num_tcn_grid_times_tr = train_data[5]
            num_tcn_grid_times_va = validation_data[5]
            tcn_grid_times_tr = train_data[6]
            tcn_grid_times_va = validation_data[6]
            num_obs_times_tr = train_data[7]
            num_obs_times_va = validation_data[7]
            num_obs_values_tr = train_data[8]
            num_obs_values_va = validation_data[8]
            ...
            if decay_learning_rate:
                learning_rate = tf.train.exponential_decay(learning_rate, global_step, decay_step, 0.96, staircase=True)
            ...
            if calculated_length > min_pad_length:
                print('Timeseries min_pad_length: {} are too short for Specified TCN Parameters requiring {}'.format(
            ...
            if losstype == 'average':
                losses = get_losses(Y, T, X, ind_kf, ind_kt, num_obs_times, num_obs_values, num_tcn_grid_times, cov_grid,
                                    input_dim,
                                    method=method, gp_params=gp_params, tcn=tcn, is_training=is_training, n_classes=n_classes,
                                    lab_vitals_only=lab_vitals_only, pad_before=pad_before,
            ...
            preds = get_preds(Y, T, X, ind_kf, ind_kt, num_obs_times, num_obs_values, num_tcn_grid_times, cov_grid, input_dim,
                              method=method, gp_params=gp_params, tcn=tcn, is_training=is_training, n_classes=n_classes,
                              lab_vitals_only=lab_vitals_only, pad_before=pad_before, losstype=losstype)  # med_cov_grid
            ...
            for i in range(training_iters):
                ...
                tr_probs_tot = np.array([])
                perm = rs.permutation(Ntr)
                tr_labels_tot = np.array([])        # update_day : 2024-04-03
                # tr_labels_tot = labels_tr[perm]
                ...
                for s, e in zip(starts, ends):
                    ...
                    T_pad, Y_pad, ind_kf_pad, ind_kt_pad, X_pad, cov_pad = pad_rawdata_nomed(
                        ...
                        tcn_grid_times_tr[inds], covs_tr[inds, :], num_tcn_grid_times_tr[inds],
                        min_pad_length)  ## meds_on_grid_tr[inds],covs_tr[inds,:]

                    feed_dict = {Y: Y_pad, T: T_pad, ind_kf: ind_kf_pad, ind_kt: ind_kt_pad, X: X_pad, cov_grid: cov_pad,
                                 num_obs_times: num_obs_times_tr[inds], num_obs_values: num_obs_values_tr[inds],
                                 num_tcn_grid_times: num_tcn_grid_times_tr[inds], O: labels_tr[inds],
                                 is_training: True}  ##med_cov_grid:meds_cov_pad,
                    ...
                    try:
                        ...
                        tr_probs, tr_acc, loss_, _ = sess.run([probs, accuracy, loss, train_op], feed_dict, options=run_options)
                    ...
                    tr_labels_tot = np.concatenate([tr_labels_tot, labels_tr[inds]])
                    tr_probs_tot = np.concatenate([tr_probs_tot, tr_probs])

                    print("len(tr_labels_tot) = {}, len(tr_probs_tot) = {}".format(len(tr_labels_tot), len(tr_probs_tot)))
                    print("Batch " + "{:d}".format(batch) + "/" + "{:d}".format(num_batches) + ", took: " + "{:.3f}".format(time() - batch_start) + ", loss: " + "{:.5f}".format(loss_))

                    write_file = open(log_path + log_file, 'a+')
                    write_file.write("train_loss,{},{},{},{},{},{:d},{:.5f}\n".format(dataset['split'], dataset['hour'], dataset['kind'], horizon, kfold, i, loss_))
                    write_file.close()
                    ...
                    if total_batches % test_freq == 0:  # Check val set every so often for early stopping
                        ...
                        va_labels_tot = np.array([])                # update_day : 2024-04-04
                        ...
                        for v_s, v_e in zip(va_starts, va_ends):
                            ...
                            va_feed_dict = {Y: Y_pad_va[va_inds, :], T: T_pad_va[va_inds, :], ind_kf: ind_kf_pad_va[va_inds, :],
                                            ...
                                            num_obs_values: num_obs_values_va[va_inds],
                                            num_tcn_grid_times: num_tcn_grid_times_va[va_inds],
                            ...
                            va_labels_tot = np.concatenate([va_labels_tot, labels_va[va_inds]])
                            va_probs_tot = np.concatenate([va_probs_tot, va_probs])

                        # Compute ROC curve and ROC area
                        va_fpr, va_tpr, _ = roc_curve(va_labels_tot, va_probs_tot)
                        va_roc_auc = auc(va_fpr, va_tpr)
                        va_auc = roc_auc_score(va_labels_tot, va_probs_tot)
                        va_prc = average_precision_score(va_labels_tot, va_probs_tot)

                        va_binary_probs_tot = []
                        for one_prob in va_probs_tot:
                            if one_prob > 0.5:
                                va_binary_probs_tot.append(1)
                            else:
                                va_binary_probs_tot.append(0)

                        # confusion matrix
                        va_conf_matrix = confusion_matrix(list(va_labels_tot), va_binary_probs_tot)
                        va_temp_matrix = list(va_conf_matrix[0]) + list(va_conf_matrix[1])
                        va_int_list = list(map(int, va_temp_matrix))

                        # result_columns = ["TP_(1_1)", "FN_(1_0)", "FP_(0_1)", "TN_(0_0)"]
                        va_tp = va_int_list[3]
                        va_fn = va_int_list[2]
                        va_fp = va_int_list[1]
                        va_tn = va_int_list[0]

                        va_best_val = max(va_prc, va_best_val)
                        va_best_auroc = max(va_auc, va_best_auroc)

                        # Plot ROC curve
                        plt.figure()
                        plt.plot(va_fpr, va_tpr, color='darkorange', lw=2, label='ROC curve (area = %0.2f)' % va_roc_auc)
                        ...
                        # Save your plot
                        va_result_image_path = result_image_path + '/va/'
                        if not os.path.isdir(va_result_image_path):
                            os.mkdir(va_result_image_path)

                        va_roc_auc_png = va_result_image_path + "/va_roc_curve_{}_{}_split_{}_hor_{}_epoch_{}_{}.png".format(
                            dataset['hour'], dataset['kind'], dataset['split'], horizon, i, total_batches)
                        plt.savefig(va_roc_auc_png)
                        ...
                        write_file = open(log_path + log_file, 'a+')
                        write_file.write("val_loss,{},{},{},{},{},{:d},{:.5f},{:.5f},{:.5f},{},{},{},{}\n".format(dataset['split'], dataset['hour'], dataset['kind'], horizon, kfold, i, va_loss, va_prc, va_auc, va_tp, va_fn, va_fp, va_tn))
                        write_file.close()

                        print("val_loss, split = {}, hour = {}, kind = {}, horizon = {}, kfold = {}, i = {:d}, va_loss = {:.5f}, va_prc = {:.5f}, va_auc = {:.5f}, va_tp = {}, va_fn = {}, va_fp = {}, va_tn = {}\n".format(
                                    dataset['split'], dataset['hour'], dataset['kind'], horizon, kfold, i,
                                    va_loss, va_prc, va_auc, va_tp, va_fn, va_fp, va_tn))
                        ...
                        saver.save(sess, checkpoint_path + "/model_{}_{}_split_{}_hor_{}_kfold_{}_epoch_{}".format(dataset['hour'], dataset['kind'], dataset['split'], horizon, kfold, i), global_step=total_batches)

                tr_fpr, tr_tpr, _ = roc_curve(tr_labels_tot, tr_probs_tot)
                tr_roc_auc = auc(tr_fpr, tr_tpr)

                tr_auc = roc_auc_score(tr_labels_tot, tr_probs_tot)
                tr_prc = average_precision_score(tr_labels_tot, tr_probs_tot)
                tr_best_val = max(tr_prc, tr_best_val)

                # Plot ROC curve
                plt.figure()
                plt.plot(tr_fpr, tr_tpr, color='darkorange', lw=2, label='ROC curve (area = %0.2f)' % tr_roc_auc)
                plt.plot([0, 1], [0, 1], color='navy', lw=2, linestyle='--')  # Random guess line
                plt.xlim([0.0, 1.0])
                plt.ylim([0.0, 1.05])
                plt.xlabel('False Positive Rate')
                ...
                # Save your plot
                tr_result_image_path = result_image_path + '/tr/'
                if not os.path.isdir(tr_result_image_path):
                    os.mkdir(tr_result_image_path)

                tr_roc_auc_png = tr_result_image_path + "/tr_roc_curve_{}_{}_split_{}_hor_{}_kfold_{}_epoch_{}_{}.png".format(
                    dataset['hour'], dataset['kind'], dataset['split'], horizon, kfold, i, total_batches)
                plt.savefig(tr_roc_auc_png)
                plt.close()  # closes the current figure

                write_file = open(log_path + log_file, 'a+')
                write_file.write(
                    "final_train_loss,{},{},{},{},{},{:d},{:.5f},{:.5f},{:.5f}\n".format(dataset['split'], dataset['kind'],
                                                                                      dataset['split'], horizon, kfold, i,
                                                                                      loss_, tr_prc, tr_auc))
                write_file.write(
                    "best,{},{},{},{},{},{},{},{:.5f},{:.5f}\n".format(dataset['split'], dataset['kind'], dataset['split'],
                                                                    horizon, kfold, labels_tr.shape, labels_va.shape,
                                                                    va_best_val, va_best_auroc))
                write_file.close()

                print("Finishing epoch " + "{:d}".format(i) + ", took " + "{:.3f}".format(time() - epoch_start) + "horizon = " + str(horizon) + ", kfold = " + str(kfold))
                print("best,{},{},{},{},{},{:.5f},{:.5f}\n".format(dataset['split'], horizon, kfold, labels_tr.shape, labels_va.shape, va_best_val, va_best_auroc))
                print("final_train_loss,{},{},{},{:d},{:.5f},{:.5f},{:.5f}\n".format(dataset['split'], horizon, kfold, i, loss_, tr_prc, tr_auc))

            # 초기화
            gc.collect()
            tf.keras.backend.clear_session()  # 모든 변수와 그래프를 초기화합니다.

    return {'Best AUPRC': tr_best_val, 'Best Validation AUPRC': va_best_val}
