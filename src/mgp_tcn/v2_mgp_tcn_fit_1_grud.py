...
ex = Experiment('V2_MGP-TCN-1-GRU_D')
...
def mpg_tcn_config():
    dataset = {
        ...
        'datapath': 'output_imputation/output_1/',
        ...
        'LOG_FILE': 'v2_1_grud_0_55h_ex1c.txt',
        'hour': '55h',
        'kind': 'ex1c'
    }
    #Padding method:
...
def get_dataset(na_thres, overwrite, horizon, data_sources, hour, kind, min_length, max_length, split, num_obs_thres, root_path, output_path):
    print('----- get_dataset ------------------------------------------------------------')
    print('------------------------------------------------------------------------------')
    print('------------------------------------------------------------------------------')
...
def fit_mgp_tcn(decomposition_method, add_diag, losstype, n_hidden, levels, kernel_size, n_mc_smps, dropout, reduction_dim, batch_size, learning_rate, decay_learning_rate,
    ...
    checkpoint_path = dataset['root_path'] + 'model_v2/checkpoints_1_grud'
    if not os.path.isdir(checkpoint_path):
        os.mkdir(checkpoint_path)
        
    checkpoint_path = checkpoint_path + '/' + checkpoint_folder  

    result_image_path = dataset['root_path'] + 'model_v2/result_image_1_grud'
    print("---------------------------------------------------------------------------------------------------")
    ...
    # Load dataset
    full_dataset = get_dataset(dataset['na_thres'], dataset['overwrite'], dataset['horizon'], dataset['data_sources'], 
    ...
    for i in range(training_iters):
        ...
        # print('Max Memory Usage up to now')
        # print(sess.run(tf.contrib.memory_stats.MaxBytesInUse()))  # GPU 관련 사용 안 할 경우 주석 처리
        ...
        for s, e in zip(starts, ends):
            ...
            write_file.write("train_loss,{},{},{},{},{:d},{:.5f}\n".format(dataset['split'], dataset['hour'], dataset['kind'], dataset['horizon'], i, loss_))
            write_file.close()
            ...
            if total_batches % test_freq == 0: # Check val set every so often for early stopping
                ...
                va_roc_auc_png = va_result_image_path + "/va_roc_curve_{}_{}_split_{}_hor_{}_epoch_{}_{}.png".format(dataset['hour'], dataset['kind'], dataset['split'], dataset['horizon'], i, total_batches)
                ...
                write_file = open(log_path + log_file, 'a+')
                write_file.write("val_loss,{},{},{},{},{:d},{:.5f},{:.5f},{:.5f}\n".format(dataset['split'], dataset['hour'], dataset['kind'], dataset['horizon'], i, va_loss, va_prc, va_auc))
                write_file.close()
                ...
                saver.save(sess, checkpoint_path + "/model_{}_{}_split_{}_hor_{}_epoch_{}".format(dataset['hour'], dataset['kind'], dataset['split'], dataset['horizon'], i), global_step=total_batches)
        ...
        tr_roc_auc_png = tr_result_image_path + "/tr_roc_curve_{}_{}_split_{}_hor_{}_epoch_{}_{}.png".format(dataset['hour'], dataset['kind'], dataset['split'], dataset['horizon'], i, total_batches)
        plt.savefig(tr_roc_auc_png)

        plt.close() # closes the current figure

        write_file = open(log_path + log_file, 'a+')
        write_file.write("final_train_loss,{},{},{},{},{:d},{:.5f},{:.5f},{:.5f}\n".format(dataset['split'], dataset['kind'], dataset['split'], dataset['horizon'], i, loss_, tr_prc, tr_auc))
        write_file.write("best,{},{},{},{},{},{},{:.5f},{:.5f}\n".format(dataset['split'], dataset['kind'], dataset['split'], dataset['horizon'], labels_tr.shape, labels_va.shape, va_best_val, va_best_auroc))
        write_file.close()

        print("Finishing epoch " + "{:d}".format(i) + ", took " + "{:.3f}".format(time()-epoch_start) + ", horizon = " + str(dataset['horizon']))
        print("best,{},{},{},{},{:.5f},{:.5f}\n".format(dataset['split'], dataset['horizon'], labels_tr.shape, labels_va.shape, va_best_val, va_best_auroc))
        print("final_train_loss,{},{},{:d},{:.5f},{:.5f},{:.5f}\n".format(dataset['split'], dataset['horizon'], i, loss_, tr_prc, tr_auc))
