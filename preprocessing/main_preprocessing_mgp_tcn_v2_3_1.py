...
def load_data(test_size=0.1, horizon=0, na_thres=500, variable_start_index=5, data_sources=['labs','vitals','covs'], min_length=None, max_length=None, overwrite=False, split=0, binned=False, root_path='/home/cdss/mgp-tcn-project/', output_path='output/', hour='55h', kind='ex1c'):
    ...
    case_merge_clinical = root_path + output_path + 'case_{}_merge_{}_clinical.csv'.format(hour, kind)
    control_merge_clinical = root_path + output_path + 'control_{}_merge_{}_clinical.csv'.format(hour, kind)
    ...
    if overwrite or not os.path.isfile(labvital_outpath):
        ...
        # 결측치 데이터 보강 >> 임상실험 결측치 보강 방법 활용
        if os.path.isfile(case_merge_clinical):
            case_labvitals = pd.read_csv(case_merge_clinical, encoding='UTF-8')
        else:
            case_labvitals = fn_clinical_fill(case_labvitals, case_merge_clinical) 

        if os.path.isfile(control_merge_clinical):
            control_labvitals = pd.read_csv(control_merge_clinical, encoding='UTF-8')
        else:
            control_labvitals = fn_clinical_fill(control_labvitals, control_merge_clinical)
