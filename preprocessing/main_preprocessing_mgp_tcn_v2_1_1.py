...
def extract_window(data=None, static_data=None, onset_name=None, horizon=0):
    ...
    data['chart_time'] = data['chart_time'].astype('datetime64[ns]')
    ...
    for icuid in tqdm(ids):
        ...
        print('---------------------------------------------')
        print(pat_window)
        print(pat_window.shape)
        print(type(pat_window['chart_time']))
        # print(pat_window['chart_time'].values[0])
        # print(type(pat_window['chart_time'].values[0]))
        print('---------------------------------------------')
        print(start)
        print(type(start))
        print('---------------------------------------------')
...
# Main function:
def load_data(test_size=0.1, horizon=0, na_thres=500, variable_start_index=5, data_sources=['labs','vitals','covs'], min_length=None, max_length=None, overwrite=False, split=0, binned=False, root_path='/home/cdss/mgp-tcn-project/', output_path='output/', hour='55h', kind='ex1c'):
    ...
    labvital_outpath = root_path + output_path + 'full_labvitals_{}_{}_horizon_{}.csv'.format(hour, kind, horizon)
    # Case vitals and labs (input, output)
    case_vitals_in = root_path + output_path + 'case_{}_hourly_vitals_{}.csv'.format(hour, kind)
    case_vitals_out = root_path + output_path + 'case_{}_hourly_vitals_{}_collected.csv'.format(hour, kind)
    case_labs_in = root_path + output_path + 'case_{}_hourly_labs_{}.csv'.format(hour, kind)
    case_labs_out = root_path + output_path + 'case_{}_hourly_labs_{}_collected.csv'.format(hour, kind)

    # Control vitals and labs (input, output)
    control_vitals_in = root_path + output_path + 'control_{}_hourly_vitals_{}.csv'.format(hour, kind)
    control_vitals_out = root_path + output_path + 'control_{}_hourly_vitals_{}_collected.csv'.format(hour, kind)
    control_labs_in = root_path + output_path + 'control_{}_hourly_labs_{}.csv'.format(hour, kind)
    control_labs_out = root_path + output_path + 'control_{}_hourly_labs_{}_collected.csv'.format(hour, kind)

    # merge data save
    case_merge_origin = root_path + output_path + 'case_{}_merge_{}_origin.csv'.format(hour, kind)
    control_merge_origin = root_path + output_path + 'control_{}_merge_{}_origin.csv'.format(hour, kind)

    # merge missing value Imputation
    case_merge_delta = root_path + output_path + 'case_{}_merge_{}_delta.csv'.format(hour, kind)
    control_merge_delta = root_path + output_path + 'control_{}_merge_{}_delta.csv'.format(hour, kind)

    case_merge_grud = root_path + output_path + 'case_{}_merge_{}_grud.csv'.format(hour, kind)
    control_merge_grud = root_path + output_path + 'control_{}_merge_{}_grud.csv'.format(hour, kind)

    # Time Series output (splitted)
    compact_split_out = root_path + output_path + 'labvitals_tr_te_val_compact_{}_{}_min_length_{}_max_length_{}_horizon_{}_split_{}.pkl'.format(hour, kind, min_length, max_length, horizon, split)
    binned_split_out = root_path + output_path + 'labvitals_tr_te_val_binned_{}_{}_min_length_{}_max_length_{}_horizon_{}_split_{}.pkl'.format(hour, kind, min_length, max_length, horizon, split)

    # Static data:
    case_static_in = root_path + output_path + 'static_variables_cases.csv'
    control_static_in = root_path + output_path + 'static_variables_controls.csv'

    # Load static info (with onset hour / times)
    case_static = pd.read_csv(case_static_in, encoding='UTF-8')
    ...
    control_static = pd.read_csv(control_static_in, encoding='UTF-8')
    ...
    if overwrite or not os.path.isfile(labvital_outpath):
        ...
        # Read cases:
        case_vitals = pd.read_csv(case_vitals_out, encoding='UTF-8') #read file to pd.dataframe
        ...
        case_labs = pd.read_csv(case_labs_out, encoding='UTF-8')
        ...
        # Read controls:
        control_vitals = pd.read_csv(control_vitals_out, encoding='UTF-8') #read file to pd.dataframe
        ...
        control_labs = pd.read_csv(control_labs_out, encoding='UTF-8')
        ...
        # NULL column Delete
        case_labvitals, control_labvitals = fn_null_column_delete(case_labvitals, control_labvitals)

        list_column = ['icustay_id', 'subject_id', 'chart_time', 'sepsis_target',
                       'sysbp', 'diabp', 'meanbp', 'resprate', 'heartrate', 'spo2_pulsoxy', 'tempc',
                       'cardiacoutput', 'tvset', 'tvobserved', 'tvspontaneous', 'peakinsppressure',
                       'totalpeeplevel', 'o2flow', 'fio2', 'aniongap', 'albumin', 'bands',
                       'bicarbonate', 'bilirubin', 'creatinine', 'chloride', 'glucose', 'hematocrit',
                       'hemoglobin', 'lactate', 'platelet', 'potassium', 'ptt', 'inr', 'pt', 'sodium',
                       'bun', 'wbc', 'creatinekinase', 'ck_mb', 'fibrinogen', 'ldh', 'magnesium',
                       'calcium_free', 'po2_bloodgas', 'ph_bloodgas', 'pco2_bloodgas', 'so2_bloodgas', 'troponin_t']
        
        if horizon != 0:
            list_column = ['icustay_id', 'subject_id', 'chart_time', 'sepsis_target',
                           'sysbp', 'diabp', 'meanbp', 'resprate', 'heartrate', 'spo2_pulsoxy', 'tempc',
                           'cardiacoutput', 'tvset', 'tvobserved', 'tvspontaneous', 'peakinsppressure',
                           'totalpeeplevel', 'o2flow', 'fio2', 'aniongap', 'albumin', 'bicarbonate',
                           'bilirubin', 'creatinine', 'chloride', 'glucose', 'hematocrit', 'hemoglobin',
                           'lactate', 'platelet', 'potassium', 'ptt', 'inr', 'pt', 'sodium',
                           'wbc', 'creatinekinase', 'ck_mb', 'fibrinogen', 'ldh', 'magnesium',
                           'calcium_free', 'po2_bloodgas', 'ph_bloodgas', 'pco2_bloodgas', 'so2_bloodgas', 'troponin_t']
        ...
        case_labvitals = case_labvitals[list_column]
        control_labvitals = control_labvitals[list_column]

        case_labvitals.to_csv(case_merge_origin, index=False, encoding='UTF-8')
        control_labvitals.to_csv(control_merge_origin, index=False, encoding='UTF-8')

        # 결측치 데이터 보강 >> GRU_D 결측치 보강 방법 활용
        if os.path.isfile(case_merge_grud):
            case_labvitals = pd.read_csv(case_merge_grud, encoding='UTF-8')
            control_labvitals = pd.read_csv(control_merge_grud, encoding='UTF-8')
        else:
            case_labvitals, case_delta = fn_grud_imputaion(case_labvitals)
            control_labvitals, control_delta = fn_grud_imputaion(control_labvitals)
            
            case_labvitals.to_csv(case_merge_grud, index=False, encoding='UTF-8')
            control_labvitals.to_csv(control_merge_grud, index=False, encoding='UTF-8')

            case_delta.to_csv(case_merge_delta, index=False, encoding='UTF-8')
            control_delta.to_csv(control_merge_delta, index=False, encoding='UTF-8')
        ...
        full_labvitals.to_csv(labvital_outpath, index=False, encoding='UTF-8')
    ...
    full_labvitals = pd.read_csv(labvital_outpath, encoding='UTF-8')

    # index 컬럼이 있는 경우 삭제
    if "index" in full_labvitals.columns:
        full_labvitals.drop(labels="index", axis=1, inplace=True)

    # horizon > 0 이상일 경우 "bun" 컬럼 삭제
    if horizon > 0 and "bun" in full_labvitals.columns:
        full_labvitals.drop(labels="bun", axis=1, inplace=True)

    print("-------------------------------------------------------------------------------------------------------")
    print(full_labvitals.columns)
    print("-------------------------------------------------------------------------------------------------------")
    ...
    if 'covs' in data_sources: # if covs required: process them
        ...
        full_static = pd.concat([case_static_sum, control_static_sum])
    ...
    pickle.dump(tvt_ids, open(root_path + output_path + 'tvt_{}_{}_info_split_{}.pkl'.format(hour, kind, split), 'wb') )
    ...
    with open(root_path + output_path + '/temporal_{}_{}_signature_info_split_{}.json'.format(hour, kind, split), 'w') as jf:
        json.dump(stats, jf)
    ...
    if binned:
        if overwrite or not os.path.isfile(binned_split_out): 
            ...
            pickle.dump(sorted_ids, open(root_path + output_path + 'tvt_{}_{}_sorted_info_split_{}.pkl'.format(hour, kind, split), 'wb') )
