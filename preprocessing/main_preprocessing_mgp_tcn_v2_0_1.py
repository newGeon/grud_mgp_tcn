...
def load_data(test_size=0.1, horizon=0, na_thres=500, variable_start_index=5, data_sources=['labs','vitals','covs'], min_length=None, max_length=None, overwrite=False, split=0, binned=False, root_path='/home/cdss/mgp-tcn-project/', output_path='output/', hour='55h', kind='ex1c'):
    ...
    # 데이터 표준화!!! (standardize 함수 >> API에 들어가야 하는 함수)
    ...
    if binned:
        ...
        #---------------------------------
        # 4. Transform Data into compact format for feeding to MGP-RNN (왜?? 이 RNN 인지 모름...)
        # MGP-RNN에 공급하기 위해 데이털르 압축 형식으로 변환
        # 이 부분 실행 됨!!!!
        ...
        if overwrite or not os.path.isfile(compact_split_out): 
            ...
            # API 개발에 들어가야 하는 함수 : compact_transform
