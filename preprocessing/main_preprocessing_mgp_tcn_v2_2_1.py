...
def load_data(test_size=0.1, horizon=0, na_thres=500, variable_start_index=5, data_sources=['labs','vitals','covs'], min_length=None, max_length=None, overwrite=False, split=0, binned=False, root_path='/home/cdss/mgp-tcn-project/', output_path='output/', hour='55h', kind='ex1c'):
    ...
    # merge missing value Enhancement
    case_merge_linear = root_path + output_path + 'case_{}_merge_{}_linear.csv'.format(hour, kind)
    control_merge_linear = root_path + output_path + 'control_{}_merge_{}_linear.csv'.format(hour, kind)
    ...
    if overwrite or not os.path.isfile(labvital_outpath):
        ...
        # 결측치 데이터 보강 >> 선형회귀 결측치 보강 방법 활용
        if os.path.isfile(case_merge_linear):
            case_labvitals = pd.read_csv(case_merge_linear, encoding='UTF-8')
            control_labvitals = pd.read_csv(control_merge_linear, encoding='UTF-8')
        else:
            case_labvitals, control_labvitals = fn_linear_regression_fill(case_labvitals, control_labvitals)
            case_labvitals.to_csv(case_merge_linear, index=False, encoding='UTF-8')
            control_labvitals.to_csv(control_merge_linear, index=False, encoding='UTF-8')
